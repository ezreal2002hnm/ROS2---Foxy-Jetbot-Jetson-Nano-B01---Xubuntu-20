# ROS2---Foxy-Jetbot-Jetson-Nano-B01---Xubuntu-20
TFG - Herminio Navarro Murcia

Sneack peek of the main .cpp
...
void JetBot::SetPID(int p,int i, int d)
{
    ...
    check(sp_blocking_write(sp, tmp, 11, sp_timeout));
}
...
void JetBot::SetParams(double linear_correction,double angular_correction)
{
    ...
    check(sp_blocking_write(sp, tmp, 9, sp_timeout));
}
...
void JetBot::SetVelocity(double x, double y, double yaw)
{
    ...
    tmp[4] = ((int16_t)(x*1000)>>8) & 0xff; tmp[5] = ((int16_t)(x*1000)) & 0xff;
    tmp[6] = ((int16_t)(y*1000)>>8) & 0xff; tmp[7] = ((int16_t)(y*1000)) & 0xff;
    tmp[8] = ((int16_t)(yaw*1000)>>8) & 0xff; tmp[9] = ((int16_t)(yaw*1000)) & 0xff;
    tmp[10] = checksum(tmp,10);
    check(sp_blocking_write(sp, tmp, 11, sp_timeout));
}
